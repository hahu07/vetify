 ...
 * per-party `CANTON_*_JWT` model canton.ts otherwise uses for the unauthenticated
 * local `dpm sandbox`.
 *
 * One shared machine identity acts as every party (actAs carries the specific
 * party per request regardless of which token authorized the call) — chosen
 * over a token-per-party model to match this project's existing single-backend-
 * process custody reality (see CLAUDE.md's Policy-Approval Security Roadmap for
 * why that's an accepted, documented simplification, not an oversight).
 *
 * Enabled by setting CANTON_OIDC_CLIENT_ID/CANTON_OIDC_CLIENT_SECRET — when
 * unset, canton.ts falls back to the legacy static-JWT-per-party flow untouched.
 */
