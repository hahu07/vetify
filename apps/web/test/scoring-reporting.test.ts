// Ported verbatim from agents/src/scoring/reporting.test.ts -- lib/scoring/
// reporting.ts's aggregatePortfolio() is a byte-for-byte port of the same
// engine, so its existing exhaustive test suite carries over unchanged
// (pure function, no DB, no fixtures needed).
import { test } from "node:test";
import assert from "node:assert/strict";
import { aggregatePortfolio } from "@/lib/scoring/reporting";
import type { MurabahahContractSnapshot } from "@/lib/scoring/reporting";
...
test("totalRepaid derives from totalDisbursed minus totalOutstanding", () => {
  const contracts: MurabahahContractSnapshot[] = [{ status: "Active", salePrice: 1_000_000, outstandingBalance: 300_000 }];
  const result = aggregatePortfolio(contracts);
...
test("defaultedCount counts Defaulted contracts, excluded from active/outstanding", () => {
  const contracts: MurabahahContractSnapshot[] = [{ status: "Defaulted", salePrice: 1_000_000, outstandingBalance: 900_000 }];
  const result = aggregatePortfolio(contracts);
